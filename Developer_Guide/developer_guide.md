...
![UI](https://github.com/cs2901/yape-bcp-project-yacket-fingerlog/blob/develop/Developer_Guide/res/Package%20UI.png)

### Component Logic

In the logical component we find the QR class which will be responsible for managing the whole system of qr, either the timer for its expiration and the number of times it can be used (max 1). The Voucher class is presented which is the father of 2 classes that will be presented later, in the voucher class we will have the generated qr, and the data of both the generating company and the requesting client, will have the list of items and the total sum of the cost of the items among others. An inheritance of the Voucher class is a ticket which requires the attributes of the voucher class in addition to the client's ID. In the invoice class the same applies as in the ticket with the difference that we will request the ruc, the address and the social reason. A vital class so that all this can take place is the authentication which will be applied in the login. And finally we have the controller class that will be the orchestrator or respository for each of the requests.
 ![componentLogic](https://github.com/cs2901/yape-bcp-project-yacket-fingerlog/blob/develop/Developer_Guide/res/Package%20Logic(corregido).png)
